...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:variable name="collection">
        <xsl:choose>
            <xsl:when
                test="contains($BookFrontMatter//SeriesInfo/SeriesTitle, 'IFIP Advances')"
                >AICT</xsl:when>
            <xsl:when
                test="contains($BookFrontMatter//SeriesInfo/SeriesTitle, 'Lecture Notes in Computer Science')"
                >LNCS</xsl:when>
            <xsl:when
                test="contains($BookFrontMatter//SeriesInfo/SeriesTitle, 'Lecture Notes in Business Information')"
                >LNBIP</xsl:when>
                        ...
                        <biblFull>
                            ...
                            <sourceDesc>
                                <biblStruct>
                                    ...
                                    <monogr>
                                        <xsl:apply-templates select="$BookFrontMatter//BookInfo/BookElectronicISBN"/>
                                        <xsl:apply-templates select="$BookFrontMatter//BookInfo/BookPrintISBN"/>
                                        ...
                                        <imprint>
                                            ...
                                            <biblScope unit="serie">
                                                <xsl:apply-templates select="$BookFrontMatter//BookInfo/BookTitle"/>
                                    ...
                                    <xsl:apply-templates select="$BookFrontMatter//SeriesInfo"/>
                                    <xsl:apply-templates
    ...
    <!-- info sur la conference -->
    <xsl:template match="ConferenceInfo">
      <meeting> 
         <title>
             
             <xsl:choose>
                 <xsl:when test="starts-with(./ConfSeriesName,./ConfNumber)"/>
                 <xsl:when test="./ConfNumber=1">
                     <xsl:text>1st </xsl:text>
                 </xsl:when>
                 <xsl:when test="./ConfNumber=2">
                     <xsl:text>2nd </xsl:text>
                 </xsl:when>
                 <xsl:when test="./ConfNumber=3">
                     <xsl:text>3rd </xsl:text>
                 </xsl:when>
                 <xsl:otherwise>
                     <xsl:value-of select="./ConfNumber"/>
                     <xsl:text>th </xsl:text>
                 </xsl:otherwise>
              </xsl:choose>
             <xsl:value-of select="normalize-space(./ConfSeriesName)"/>
             <xsl:text> (</xsl:text>
             <xsl:value-of select="./ConfEventAbbreviation"/>
             <xsl:text>)</xsl:text>
         </title>
        <date type="start">
        ...
        <date type="end">
          ...
          <settlement><xsl:apply-templates select="./ConfEventLocation/City"/></settlement>
          <country key="$resultCode">
              <xsl:apply-templates select="./ConfEventLocation/Country"/>
          </country>      
      </meeting>
       
    </xsl:template>
    <!-- affiche une date de debut ou fin de conf -->
    <xsl:template match="ConfEventDateStart|ConfEventDateEnd">
